...
class weChat
{
    ...
    {
        $access_token = $this->mem->getMemcache($this->appid);
        if (!$access_token) {
            $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$this->appid}&secret={$this->appsecret}";
            $request = $this->curl->httpRequest($url);
            var_dump($request);
            $access_token = $request['access_token'];
            $this->mem->setMemcache($this->appid, $access_token, 7000);
        }
        return $access_token;
    }
    ...
    public function sendText($text)
    {
        ...
        $users = ['oRwYs6IVGuNN-kRmTVuo8h6SXi7U', 'oRwYs6O-6pXP780bZ7TYzAjTAixo'];
        $data = [
            'touser' => $users,
            'msgtype' => 'text',
            'text' => ['content' => $text],
    ...
    // 发送模板消息
    public function sendTmp($user, $tmp_id, $arr)
    {
        ...
        $arr = array(
            ...
            "data" => array(
                "num" => array("value" => $arr['num'], "color" => '#f00'),
